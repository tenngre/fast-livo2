...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<double>("preprocess/filter_size_surf", filter_size_surf_min, 0.8); // 增大下采样分辨率，减少点云数量
  nh.param<bool>("preprocess/hilti_en", hilti_en, false);
  nh.param<int>("preprocess/lidar_type", p_pre->lidar_type, AVIA);
  nh.param<int>("preprocess/scan_line", p_pre->N_SCANS, 6);
  nh.param<int>("preprocess/point_filter_num", p_pre->point_filter_num, 5); // 增大点云过滤间隔，减少点云数量
  nh.param<bool>("preprocess/feature_extract_enabled", p_pre->feature_enabled, false); // 默认关闭特征提取，减少计算量
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  sub_pcl = p_pre->lidar_type == AVIA ? 
            nh.subscribe(lid_topic, 10000, &LIVMapper::livox_pcl_cbk, this): 
            nh.subscribe(lid_topic, 10000, &LIVMapper::standard_pcl_cbk, this);
  sub_imu = nh.subscribe(imu_topic, 10000, &LIVMapper::imu_cbk, this);
  sub_img = nh.subscribe(img_topic, 1000, &LIVMapper::img_cbk, this);
  
  // 降低发布队列大小，减少内存占用
  pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered", 50);
  pubNormal = nh.advertise<visualization_msgs::MarkerArray>("visualization_marker", 50);
  pubSubVisualMap = nh.advertise<sensor_msgs::PointCloud2>("/cloud_visual_sub_map_before", 50);
  pubLaserCloudEffect = nh.advertise<sensor_msgs::PointCloud2>("/cloud_effected", 50);
  pubLaserCloudMap = nh.advertise<sensor_msgs::PointCloud2>("/Laser_map", 50);
  ...
  pubLaserCloudDyn = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj", 50);
  pubLaserCloudDynRmed = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj_removed", 50);
  pubLaserCloudDynDbg = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj_dbg_hist", 50);
  mavros_pose_publisher = nh.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose", 10);
  pubImage = it.advertise("/rgb_img", 1);
  pubImuPropOdom = nh.advertise<nav_msgs::Odometry>("/LIVO2/imu_propagate", 1000);
  imu_prop_timer = nh.createTimer(ros::Duration(0.004), &LIVMapper::imu_prop_callback, this);
  voxelmap_manager->voxel_map_pub_= nh.advertise<visualization_msgs::MarkerArray>("/planes", 1000);
}
...
{
  // 预计算变换矩阵，减少重复计算
  const Eigen::Matrix3d combined_rot = rot * extR;
  const Eigen::Vector3d combined_t = rot * extT + t;
  ...
  for (size_t i = 0; i < input_cloud->size(); i++)
  {
    ...
    p = combined_rot * p + combined_t;
    
    auto& pi = trans_cloud->points[i];
    pi.x = p(0);
    pi.y = p(1);
    pi.z = p(2);
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  static int visualization_counter = 0;
  pub_num++;
  visualization_counter++;

  // 减少可视化发布频率，每2帧发布一次
  if (visualization_counter % 2 != 0) {
    // 只清空缓冲区，不发布
  ...
  if (LidarMeasures.lio_vio_flg == VIO)
  {
    *pcl_wait_pub += *pcl_w_wait_pub;
    if(pub_num >= pub_scan_num)
    {
      ...
      laserCloudWorldRGB->reserve(size / 2); // 预分配减半的空间
      // double inv_expo = _state.inv_expo_time;
      cv::Mat img_rgb = vio_manager->img_rgb;
      
      // 随机采样点云，减少发布数量
      for (size_t i = 0; i < size; i += 2) // 每隔一个点采样一次
      {
  ...
  { 
    // 对LIO模式下的点云进行下采样，减少发布数量
    PointCloudXYZI::Ptr downsampled_cloud(new PointCloudXYZI());
    pcl::VoxelGrid<PointType> voxel_filter;
    voxel_filter.setInputCloud(pcl_w_wait_pub);
    voxel_filter.setLeafSize(0.2f, 0.2f, 0.2f); // 0.2m的下采样分辨率
    voxel_filter.filter(*downsampled_cloud);
    pcl::toROSMsg(*downsampled_cloud, laserCloudmsg); 
  }
